...
object SerializeCorefOffsetsData {
    ...
    def serializeCorefOffsetsData (year: String){
      ...
      val pw = new PrintWriter(new File("./src/main/resources/edu/knowitall/tac2013/enitylinking/coref/"+year+"corefmentions.txt"))
	    pw.close()
	    for(q <- queries){
	       val fw = new FileWriter("./src/main/resources/edu/knowitall/tac2013/enitylinking/coref/"+year+"corefmentions.txt",true)
	    	  fw.write(q.id)
	          val corefIntervals = KBPQuery.corefHelper.getCorefIntervals(SolrHelper.getRawDoc(q.doc),q.begOffset)
	          for(cmi <- scala.collection.JavaConversions.asScalaIterable(corefIntervals)){
	             fw.write("\t" +cmi)
